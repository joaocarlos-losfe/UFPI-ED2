...
void listInsertOrdered(List list, Data data) 
{
    ...
    while ((current!=NULL) && (current->linha < data->linha)) 
    {
        previous = current;
        current = current->next;
    }
...
Data listRemoveValue(List list, int linha) 
{
    ...
    if (current != NULL) 
    {
        ...
        while ((current != NULL) && current->linha != linha) 
        {
            previous = current;
            current = current->next;
        }
...
Data ListSeach(List list, int linha)
{
    Data current = list->head;
