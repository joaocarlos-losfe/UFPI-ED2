...
void listInsertOrdered(List list, Data data) 
{
    ...
    while ((current!=NULL) && (current->str_value[0] < data->str_value[0])) 
    {
        previous = current;
        current = current->next;
    }
...
Data listRemoveValue(List list, char *str_value) 
{
    ...
    if (current != NULL) 
    {
        ...
        while ((current != NULL) && strcmp(current->str_value, str_value) != 0) 
        {
            previous = current;
            current = current->next;
        }
...
Data listSeach(List list, char *wold) 
{
    ...
    else 
    {
        ...
        while(data != NULL && !is_seach) 
        {
            if (strcmp(data->str_value, wold) == 0)
                is_seach = true;
            else
                data = data->next;
        }
